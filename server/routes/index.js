...
/* const geojsonQuery =
  "SELECT row_to_json(fc) FROM ( SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features FROM (SELECT 'Feature' As type, ST_AsGeoJSON(lg.geom)::json As geometry, row_to_json((obj_name, obj_authors, obj_year, type_of_work, obj_assoc_inv_nums, path_cloud, lat_center, lon_center, stor_folder)) As properties FROM \"uds_meta_view_mat_v4\" As lg) As f) As fc";
 */
const geojsonQuery =
  "SELECT row_to_json(fc) FROM (SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features FROM (SELECT 'Feature' As type, ST_AsGeoJSON(lg.geom)::json As geometry, object, row_to_json((array_to_string(array_agg(obj_name), '; '), array_to_string(array_agg(obj_authors), '; '), array_to_string(array_agg(obj_year), '; '), array_to_string(array_agg(type_of_work), '; '), array_to_string(array_agg(obj_group_min), '; '), array_to_string(array_agg(path_cloud), '; '), array_to_string(array_agg(lat_center), '; '), array_to_string(array_agg(lon_center), '; '), array_to_string(array_agg(stor_folder), '; ') )) As properties FROM uds_meta_view_mat_v4 As lg group by geom, object, area_skm order by area_skm desc) As f)As fc;";
